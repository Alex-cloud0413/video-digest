...
YT Digest is a small GitHub-only project. Security fixes are made on the latest code on `main` and, when releases are published, the latest GitHub release. Older snapshots are not supported.

## Report a vulnerability privately

Do not open a public issue for a vulnerability, exposed credential, or report containing private video or transcript data.
